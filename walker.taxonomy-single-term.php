<?php
if ( ! class_exists( 'Taxonomy_Single_Type_Walker' ) && class_exists( 'Walker' ) ) :
/**
 ...
 * @since 0.1.2
 */
class Taxonomy_Single_Type_Walker extends Walker {
	...
	public function start_el_select( $args ) {
		return "\n".sprintf(
			'<option %s %s id="%s" value="%s" class="class-single-type">%s',
			$args['selected'],
			$args['disabled'],
			$args['id'],
			$args['value'],
